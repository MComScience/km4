...
?>
<?php $form = ActiveForm::begin(['type' => ActiveForm::TYPE_HORIZONTAL, 'id' => 'formdetailnd_cont']); ?>
...
<div class="well">
    <div class="row">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    ...
                    $form->field($modeledit, 'PCPlanNum', ['showLabels' => false])->widget(Select2::classname(), [
                        'data' => ArrayHelper::map(app\modules\Inventory\models\VwNdplanDetailAvalible::find()->where(['PCPlanNum' => $PCPlanNum])->all(), 'PCPlanNum', 'PCPlanNum'),
                        'language' => 'en',
                        'pluginOptions' => [
        ...
        <input type="hidden" id="vvwpritemdetail2temp-ids" name="VwPritemdetail2Temp[TMTID_GPU]" value="<?php echo $modeledit['TMTID_GPU']; ?>"/>
        <div class="col-md-12">
...
$(document).ready(function () {
    ...
    $('#vwpritemdetail2temp-itempackid').on('change', function () {
        var ItemID = $("#vwpritemdetail2temp-itemid").val();
        ...
        if (ItemPackUnit != '') {
            $.ajax({
                url: "index.php?r=Purchasing/addpr-nd-cont/get-qtynd",
                type: "post",
    ...
    $('#vwpritemdetail2temp-pcplannum').on('change', function () {
        ...
        if (PCPlanNum != '') {
            $.post({
                url: "index.php?r=Purchasing/addpr-nd-cont/getdata-pcplannd", // your controller action
                dataType: 'json',
                data: {PCPlanNum: PCPlanNum, ItemID: ItemID},
                success: function (data) {
                    $('#vwpritemdetail2temp-itemname').val(data.ItemName);
                    $('#vwpritemdetail2temp-pritemunitcost').val(data.PCPlanNDUnitCost);
                    $('#vwpritemdetail2temp-pritemorderqty').val(data.PCPlanNDQty);
                    $('#vwpritemdetail2temp-prapprovedorderqtysum').val(data.PRApprovedQtySUM);
                    $('#vwpritemdetail2temp-pritemavalible').val(data.PRNDAvalible);
                    $('#vwpritemdetail2temp-pritemstdcost').val(data.NDStdCost);
                },
            });
        }
    });
...
$("#vwpritemdetail2temp-itempackcost").keyup(function () {
    ...
    //On Save
$('#formdetailnd_cont').on('beforeSubmit', function (e)
{
                        ...
                        closeOnCancel: true,
                            ...
                            function (isConfirm) {
                                if (isConfirm) {
                                    ...
                                    $.pjax.reload({container: '#nd_cont_pjax_id'});
                                } else {
                                    l.ladda('stop');
                                    //$('#nd-modal').modal('hide');
                                    //$('#formdetailnd_cont').trigger("reset");
