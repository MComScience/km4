                ...
                <div id="home" class="tab-pane in active">
                    <div class="well">
                        <div class="tb-pr2-form">
                            <?php
                            $form = ActiveForm::begin(['type' => ActiveForm::TYPE_HORIZONTAL, 'id' => 'formtempnewnd']);
                            ?>
                            ...
                            <?php if ($view == '') { ?>
                                <div class="form-group">
                                    <a class="btn btn-success" id="createitemidtemp"><i class="glyphicon glyphicon-plus">เพิ่มรายการเวซภัณฑ์ฯ</i></a>
                                </div>
                            <?php } ?>
                            <?php if ($view == '') { ?>
                                <div class="form-group">
                                    <?php \yii\widgets\Pjax::begin([ 'id' => 'nd_detail_id']) ?>
                                    <?php
...
//                                                },
        ...
        \yii\bootstrap\Modal::begin([
            'id' => 'getdatandmodal',
            'header' => '<h4 class="modal-title">เลือกรายการเวซภัณฑ์ฯ</h4>',
        ...
        \yii\bootstrap\Modal::begin([
            'id' => 'nd-modal',
            'header' => '<h4 class="modal-title">บันทึกรายการเวซภัณฑ์ฯ รายการใหม่</h4>',
            'size' => 'modal-lg modal-primary',
        ]);
        ?>
        <div id="data"></div>
...
$('#createitemidtemp').click(function (e) {
               var PRID = $("#tbpr2temp-prid").val();
               var ids_PR_selected = $("#tbpr2temp-ids_pr_selected").val();
                $.ajax({
                url: 'index.php?r=Purchasing/new-pr-nd/create-itemidtemp',
                        type: 'POST',
                        data: {PRID: PRID},
                        success: function (data) {
                        if (data == '') {
                            Notify('ไม่มีอะไรเลย!', 'top-right', '2000', 'danger', 'fa-exclamation', true);
                        } else {
...
function init_click_handlers() {
    ...
    $.get(
                'index.php?r=Purchasing/new-pr-nd/update-detailnd',
                {
                    id: fID
                },
        function (data)
        {   
            ...
            $('#data').html(data);
            $('.modal-title').html('ปรับปรุงรายการเวซภัณฑ์ ใบขอซื้อ');
            $('#nd-modal').modal('show');
            $('#cmd').val('2');
          }
        );
    ...
    $('.activity-delete-link').click(function (e) {
        var fID = $(this).closest('tr').data('key');
            ...
            swal({   
                ...
                $.post(
                        'index.php?r=Purchasing/new-pr-nd/delete-detailnd',
                        {
                            id: fID
                        },
                function (data)
...
$('#nd_detail_id').on('pjax:success', function () {
    init_click_handlers(); //reactivate links in grid after pjax update
});
//On Save
    $('#formtempnewnd').on('beforeSubmit', function(e)
    {
    wait();
    var \$form = $(this);
            $.post(
...
$(document).ready(function () {
                    $('#tbpr2temp-prtypeid').on('change', function () {
                        ...
                        $.ajax({
                            url: 'index.php?r=Purchasing/new-pr-nd/get-reasonnd',
                            type: 'POST',
                            data: {PRType: PRType},
                            dataType: 'json',
                            success: function (data) {
    ...
    $('#SaveDraft').click(function (e) {
        ...
        //if (reasonid != "") {
            $.post(
                    'index.php?r=Purchasing/new-pr-nd/save-reason',
                    {
    ...
    $('#Clear').click(function (e) {
        var PRID = $("#tbpr2temp-prid").val();
            ...
            swal({   
                ...
                $.post(
                        'index.php?r=Purchasing/new-pr-nd/clear-nd',
                        {
                            PRID: PRID
                        },
                function (data)
...
JS;
            ...
            <script>
                ...
                $.post(
                        'index.php?r=Purchasing/new-pr-nd/sendtoverify',
                {
