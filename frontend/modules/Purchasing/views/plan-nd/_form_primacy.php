...
$this->title = 'ปรับปรุงแผนจัดชื้อเวชภัณฑ์' . ' ';
$model->PCPlanNum;
            ...
            if ($tbpcplangpu != "") {
                ...
                foreach ($tbpcplangpu as $result) {
                    ...
                    if ($type == 'pharmacist') {
                        ...
                        $htl .='<td align="center"><a href="javascript:editlistnondrug(' . $result['ids'] . ')"  class="btn btn-info btn-xs edit disabled"> Edit</a> <a href="javascript:deletedetailnondrug(' . $result['ids'] . ')"  class="btn btn-danger btn-xs delete disabled"> Delete</a></td>';
                    }
                    $htl .='</tr>';
                    $no++;
...
<script>
    ...
    function showselectdata() {
        if ($('#inputEmail3').val() == "") {
            ...
            l.ladda('start');
            $.ajax({
                url: "index.php?r=Purchasing/plan-tpu/data1",
                type: "post",
                success: function (d) {
                    $('#dataread').html(d);
    ...
    function myFunselectdata(id) {
        ...
        $.ajax({
            url: 'index.php?r=Purchasing/plan-tpu/getdata',
            type: "post",
    ...
    function sendtoapprove() {
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                $.ajax({
                    url: "index.php?r=Purchasing/plan-tpu/send-to-approve",
                    type: "post",
                    data: {id: id},
                    dataType: 'json',
                    success: function (result) {
    ...
    function approve(type) {
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                l.ladda('start');
                $.ajax({
                    url: "index.php?r=Purchasing/tbplan/approve",
                    type: "post",
                    ...
                    success: function (result) {
                        ...
                        setTimeout("location.href = 'index.php?r=Purchasing/plan-tpu/wailt-approve';", 10);
                    }
                });
            }
        }
    ...
    function saveactive() {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var id = $("#inputEmail13").val();
                $.ajax({
                    url: "index.php?r=Purchasing/plan-tpu/saveactive",
                    type: "post",
                    data: {id: id},
                    dataType: 'json',
                    success: function (result) {
    ...
    function  Save() {
        if ($("#NonDrugUnitCost").val() == "" || $("#NonDrugUnitCost").val() < 0.01) {
            ...
            l.ladda('start');
            $.ajax({
                url: "index.php?r=Purchasing/plan-tpu/save",
                type: "post",
    ...
    function editlistnondrug(id) {
        $.ajax({
            url: "index.php?r=Purchasing/plan-tpu/editlistnondrug",
            type: "post",
            data: {id: id},
            dataType: 'json',
            success: function (result) {
    ...
    function deletedetailnondrug(ids) {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var prnums = $("#inputEmail13").val();
                $.ajax({
                    url: "index.php?r=Purchasing/plan-tpu/deletedetailnondrug",
                    type: "post",
                    ...
                    success: function (result) {
                        ...
                        // Notify(result.ff, 'top-right', '5000', 'success', 'fa-check', true);
                    }
                });
            }
        }
...
//Notify('บันทึกรายการแล้ว', 'top-right', '5000', 'success', 'fa-check', true);
