...
<div class="well">
    <div class="row">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    ...
                    $form->field($modeledit, 'PCPlanNum', ['showLabels' => false])->widget(Select2::classname(), [
                        'data' => ArrayHelper::map(VwGpuplanDetailAvalible::find()->where(['PCPlanNum' => $PCPlanNum])->all(), 'PCPlanNum', 'PCPlanNum'),
                        'language' => 'en',
                        'pluginOptions' => [
        ...
        <div class="col-xs-6">
            ...
            <div class="form-group">
                <label class="col-sm-4 control-label no-padding-right">ราคาต่อหน่วย <a
                        id="checkราคาต่อหน่วย"></a></label>
                <div class="col-sm-8">
                    <?=
                    $form->field($modeledit, 'PRUnitCost', ['showLabels' => false])->textInput([ 'value' => number_format($PRUnitCost, 2),
            ...
            <?= $form->field($modeledit, 'PRID', ['showLabels' => false])->hiddenInput([]) ?>
        </div>
...
$('#vwpritemdetail2-pcplannum').on('change', function () {
    ...
    if(PCPlanNum != ''){
        $.ajax({
        url: 'index.php?r=Purchasing/addpr-gpu/getdata-pcplangpu',
        type: 'POST',
        dataType: 'json',
        data: {PCPlanNum: PCPlanNum, TMTID_GPU: TMTID_GPU},
        success: function (data) {
            $('#vwpritemdetail2-itemname').val(data.FSN_GPU);
            $('#vwpritemdetail2-pritemstdcost').val(data.GPUStdCost);
            $('#vwpritemdetail2-pritemunitcost').val(data.GPUUnitCost);
            $('#vwpritemdetail2-pritemorderqty').val(data.GPUOrderQty);
            $('#vwpritemdetail2-prapprovedorderqtysum').val(data.PRApprovedOrderQty);
            $('#vwpritemdetail2-pritemavalible').val(data.PRGPUAvalible);
        }
    });
    }  
});
    ...
    $.post(
            ...
            .done(function (result) {
                ...
                } else if (result == 'overall')
                {
                    ...
                    $('#form_update_verify').trigger("reset");
                    swal("Save Complete!", "", "success");
                    $('#SaveDraft').removeClass("disabled", "disabled");
                }
            })
            .fail(function ()
            {
