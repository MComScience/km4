 ...
 */
class NewPrTpuController extends Controller {
...
//$htl .= '</label>';
     ...
     function actionGetdatatpu() {
        $Productmodel = \app\models\TbMastertmt::find()->all();
        $htl = '<table class="table table-striped  table-bordered dt-responsive " cellspacing="0" width="100%" id="getdatatputable">
        ...
        foreach ($Productmodel as $result) {
            ...
            $htl .='<td style="text-align: center;"><a class="btn btn-success btn-sm" onclick="SelectTPU(' . $result->TMTID_TPU . ');" > select</a></td>';
            $htl .='</tr>';
            $no++;
        }
        $htl .='</tr></tbody>
     ...
     public function actionNewDetailtpu() {
        ...
        if ($modeledit->load(Yii::$app->request->post())) {
            $modeleditpack = \app\models\TbItempack::find()->where(['TMTID_GPU' => $_POST['VwPritemdetail2Temp']['TMTID_GPU'],'ItemPackUnit' => $_POST['VwPritemdetail2Temp']['ItemPackID']])->one(); $modeleditpack = \app\models\TbItempack::find()->where(['TMTID_GPU' => $_POST['VwPritemdetail2Temp']['TMTID_GPU'],'ItemPackUnit' => $_POST['VwPritemdetail2Temp']['ItemPackID']])->one();
            $cmd = $_POST['cmd'];
            $PCPlanNum1 = "";
            $ItemID = "";
            $TMTID_GPU = $_POST['VwPritemdetail2Temp']['TMTID_GPU'];
            $TMTID_TPU = $modeledit['TMTID_TPU'];
...
//return $this->redirect(['view', 'id' => $model->PRID]);
            ...
            $check = \app\modules\Purchasing\models\Tbpritemdetail2temp::findAll(['TMTID_TPU' => $_POST['id'], 'PRID' => $_POST['PRID']]);
            $checkitem = \app\models\VwItemListTpu::findAll(['TMTID_TPU' => $_POST['id']]);
            if ($check != null) {
                return 'false';
...
//get FSN_GPU on TbGenericproductuseGpu
                $Item = \app\models\TbMastertmt::findOne(['TMTID_TPU' => $_POST['id']]);
                $modeledit['ItemName'] = $Item['FSN_TMT'];
                $modeledit['TMTID_GPU'] = $Item['TMTID_GPU'];
...
//                }
     ...
     public function actionViewDetailtpu() {
        if (isset($_POST['expandRowKey'])) {
            ...
            return $this->renderPartial('view_detail_tpu', ['model' => $model, 'packunit' => $packunit, 'pack' => $pack]);
        } else {
...
//return $this->redirect(['view', 'id' => $model->PRID]);
            ...
            $itemdisp = \app\models\TbMastertmt::findOne(['TMTID_TPU'=> $modeledit['TMTID_TPU']]);
...
//GetPack
            ...
            return $this->renderAjax('_update_detailtpu', [
                        ...
                        'ItemName' => $modeledit['ItemName'],
                        'btn' => '',
                        'DispUnit' => $itemdisp['DispUnit_TMT']
            ]);
        }
    }
