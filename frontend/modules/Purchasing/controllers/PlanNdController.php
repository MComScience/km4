 ...
 */
class PlanNdController extends Controller {
    ...
    function actionApprove() {
        ...
        if ($datas != null) {
            ...
            Yii::$app->logger->savelog("อนุมัติแผนจัดชื้อเวชภัณฑ์มิใช่ยา", $id);
        } else {
            $data = array(
                'status' => '0',
    ...
    function actionSendToApprove() {
        ...
        if ($datas != null) {
            ...
            Yii::$app->logger->savelog("ส่งแผนจัดชื้อเวชภัณฑ์มิใช่ยาอนุมัติ", $id);
        } else {
            $data = array(
                'status' => '0',
                'data' => 'กรุณา Save Draf ก่อน'
...
//            }
            ...
            $model->PCPlanTotal = $totalall;
            if ($model->save()) {
                Yii::$app->logger->savelog("บันทึกแผนจัดชื้อเวชภัณฑ์มิใช่ยา", $pos['PCPlanNum']);
                echo 1;
            } else {
                echo 0;
            }
...
//            }
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                Yii::$app->logger->savelog("แก้ไขแผนจัดชื้อเวชภัณฑ์มิใช่ยา", $pos['PCPlanNum']);
                echo 1;
            } else {
                echo 0;
            }
    ...
    public function actionDelete() {
        ...
        Yii::$app->logger->savelog("ลบแผนจัดชื้อเวชภัณฑ์มิใช่ยา", $id);
