 ...
 */
class PlansaleNdController extends Controller {
    ...
    public function actionForm_main() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->PCPlanDate = Yii::$app->dateconvert->convertThaiToMysqlDate2($post['PCPlanDate']); //convert to thai
            $model->PCPlanBeginDate = Yii::$app->dateconvert->convertThaiToMysqlDate2($post['PCPlanBeginDate']);
            $model->PCPlanEndDate = Yii::$app->dateconvert->convertThaiToMysqlDate2($post['PCPlanEndDate']);
    ...
    function actionApprove() {
        ...
        if ($datas != null) {
            ...
            Yii::$app->logger->savelog("อนุมัติสัญญาจะชื้อจะขายเวชภัณฑ์มิใช่ยา", $id);
        } else {
            $data = array(
                'status' => '0',
    ...
    function actionSendToApprove() {
        ...
        if ($datas != null) {
            ...
            Yii::$app->logger->savelog("ส่งสัญญาจะชื้อจะขายเวชภัณฑ์มิใช่ยาไปอนุมัติ", $id);
        } else {
            $data = array(
                'status' => '0',
                'data' => 'กรุณา Save Draf ก่อน'
...
//    }
    ...
    public function actionCreate($PCPlanNum = null) {
        $post = Yii::$app->request->post('TbPcplan2');
        $model = TbPcplan2::findOne($PCPlanNum);
        //$model = new TbPcplan2();
        if ($model->load(Yii::$app->request->post())) {
...
//            }
            ...
            echo 1;
//            if ($model->save()) {
//                Yii::$app->logger->savelog("บันทึกสัญญาจะชื้อจะขายเวชภัณฑ์มิใช่ยา", $post['PCPlanNum']);
//                echo 1;
//            } else {
//                echo 0;
//            }
...
//            }
            ...
            $tbpcplangpu = Viewplannddetail::findAll(['PCPlanNum' => $PCPlanNum]);
            if ($tbpcplangpu == null) {
                $tbpcplangpu = "";
            }
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->PCPlanTotal = $totalall;
            if ($model->save()) {
                Yii::$app->logger->savelog("แก้ไขสัญญาจะชื้อจะขายเวชภัณฑ์มิใช่ยา", $post['PCPlanNum']);
                echo 1;
            } else {
                echo 0;
            }
    ...
    public function actionDelete() {
        ...
        Yii::$app->logger->savelog("ลบสัญญาจะชื้อจะขายเวชภัณฑ์มิใช่ยา", $id);
