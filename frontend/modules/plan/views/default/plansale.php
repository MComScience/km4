...
$this->title = 'สัญญาจะซื้อจะขาย';
$this->params['breadcrumbs'][] = ['label' => 'Purchasing', 'url' => ['plansale']];
$this->params['breadcrumbs'][] = ['label' => 'แผนการจัดชื้อ', 'url' => ['plansale']];
...
<style type="text/css">
    ...
    td.details-control {
        background: url('/km4/vendor/DataTables-1.10.12/examples/resources/details_open.png') no-repeat center center;
        cursor: pointer;
    }
    tr.shown td.details-control {
        background: url('/km4/vendor/DataTables-1.10.12/examples/resources/details_close.png') no-repeat center center;
    }
</style>
<div class="row">
                ...
                <table class="default kv-grid-table table table-hover table-condensed kv-table-wrap" width="100%">
                    <thead>
                        <tr>
                            ...
                            <?= Html::tag('th', Html::encode('เลขที่สัญญาจะซื้อจะขาย'), ['style' => $style]) ?>

                            <?= Html::tag('th', Icon::show('calendar', [], Icon::BSG) . Html::encode('วันที่'), ['style' => $style]) ?>

                            <?= Html::tag('th', Html::encode('รหัสยาการค้า'), ['style' => $style]) ?>

                            <?= Html::tag('th', Html::encode('รายละเอียดยา'), ['style' => 'border-top: 1px solid #ddd;']) ?>

                            <?= Html::tag('th', Html::encode('ราคา/หน่วย'), ['style' => 'border-top: 1px solid #ddd;']) ?>
                    ...
                    <tbody>
                        <?php foreach ($query as $v) : ?>
                            ...
                            if (($v['PCPlanStatusID'] == 2) || ($v['PCPlanStatusID'] == 1)) {
                                if (($v['PCPlanTypeID'] == 1 || $v['PCPlanTypeID'] == 2)) {
                                    ...
                                    $btnaction = '<div class="btn-group dropdown">
                                            ...
                                            . '<ul class="dropdown-menu dropdown-success">
                                                <li>
                                                    ' . Html::a(Icon::show('list-alt') . Yii::t('app', 'Detail'), ['/plan/tpu/details', 'data' => $v['PCPlanNum']], ['role' => 'modal-remote', 'style' => 'font-size:11pt;'])
                                            . '</li>
                                               <li>
                            ...
                            <tr class="<?= $v['PCPlanTypeID'] == 5 || $v['PCPlanTypeID'] == 6 ? 'defult' : 'defult' ?>">
                                ...
                                <?= Html::tag('td', empty($v['PCPOContactID']) ? '-' : $v['PCPOContactID'], ['style' => 'text-align: center;']) ?>

                                <?= Html::tag('td', empty($v['PCPlanDate']) ? '-' : Yii::$app->formatter->asDate($v['PCPlanDate'], 'dd/MM/yyyy'), ['style' => 'text-align: center;']) ?>

                                <?= Html::tag('td', '<span class="badge badge-success"><strong>'.$v['TMTID_TPU'].'</strong></span>', ['style' => 'text-align: center;']) ?>

                                <?= Html::tag('td', empty($v['FSN_TMT']) ? '-' : $v['FSN_TMT'], ['style' => 'text-align: left;']) ?>
                                
                                <?= Html::tag('td', empty($v['TPUUnitCost']) ? '-' : number_format($v['TPUUnitCost'],2), ['style' => 'text-align: right;']) ?>
                                
                                <?= Html::tag('td', empty($v['TPUOrderQty']) ? '-' : number_format($v['TPUOrderQty'],2), ['style' => 'text-align: right;']) ?>
                                
                                <?= Html::tag('td', empty($v['DispUnit']) ? '-' : $v['DispUnit'], ['style' => 'text-align: center;']) ?>

                                <?php // Html::tag('td', empty($v->department->DepartmentDesc) ? '-' : $v->department->DepartmentDesc, ['style' => 'text-align: center;']) ?>

                                <?php // Html::tag('td', empty($v->section->SectionDecs) ? '-' : $v->section->SectionDecs, ['style' => 'text-align: center;']) ?>

                                <?php // Html::tag('td', empty($v->plantype->PCPlanType) ? '-' : $v->plantype->PCPlanType, ['style' => 'text-align: center;']) ?>

                                <?= Html::tag('td', empty($v['PCPlanEndDate']) ? '-' : Yii::$app->formatter->asDate($v['PCPlanEndDate'], 'dd/MM/yyyy'), ['style' => 'text-align: center;']) ?>

                                <?= Html::tag('td', empty($v['PCPlanEndDate']) ? '-' : Yii::$app->formatter->asDate($v['PCPlanEndDate'], 'dd/MM/yyyy'), ['style' => 'text-align: center;']) ?>

                                <?= Html::tag('td', empty($v['PCPlanStatus']) ? '-' : $v['PCPlanStatus'], ['style' => 'text-align: center;']) ?>
...
<?php $this->registerJsFile(Yii::getAlias('@web') . '/js/plan/datatables-sale.js', ['depends' => [\yii\web\JqueryAsset::className()]]); ?>
