...
<ul class="nav nav-tabs tabs-flat  nav-justified" id="myTab11">
    ...
    <li>
        <a style="text-align: right">
            <b class="success">Last Dx Date : </b><?php echo Yii::$app->thaiYearformat->asDate('sort') ?> &nbsp;
            <button class="btn btn-info btn-xs"><i class="glyphicon glyphicon-list-alt"></i> Detail</button>
...
$("#treatment").click(function (e) {               
    window.location.replace("index.php?r=pharmacy/ip/treatment&data=$header->pt_visit_number");
});
$("#Patient").click(function (e) {               
    window.location.replace("index.php?r=pharmacy/ip/patient&data=$header->pt_visit_number");
});
$("#orderset").click(function (e) {               
    location.reload();
});       
$("li.order").click(function (e) {               
    location.reload();
}); 
JS;
