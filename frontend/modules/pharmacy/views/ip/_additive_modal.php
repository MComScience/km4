...
<script>
    $(document).ready(function () {
        ...
        function DisabledRate() {
            var qty = $('#tbdrugsetdetail-itemqty1').val();
            var itemstatus = $('#tbdrugsetdetail-cpoe_itemstatus').val();
            if (qty < '0' && itemstatus == '1') {
                document.getElementById("อัตราการให้").checked = true;
...
//                            (min < Item_Cr_Amt && ItemQtyAvalible < max)
    ...
    function SelectItemDrug(e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('.disunitontableadditive').html(DispUnit);
                var pt_visit_number = $('input[id=tbpttrpchemo-pt_visit_number]').val();
                $('#pt_visit_number').val(pt_visit_number);
    ...
    function GetDisunit(id) {
        $.ajax({
            url: "index.php?r=pharmacy/ip/get-disunit",
            type: "POST",
            data: {id: id},
            dataType: "JSON",
            success: function (result) {
                $('.disunitontableadditive').html(result);
            }
        });
    }
