...
<?php if ($type == 'c' && $model['PRTypeID'] == '1' || $model['PRTypeID'] == '2' || $model['PRTypeID'] == '4' || $model['PRTypeID'] == '6' || $model['PRTypeID'] == '7') : ?>
    ...
    <table width="100%" cellspacing="0" cellpadding="0" border="0">
        ...
        <tr>
            <td colspan="2" style="font-size: 16pt;line-height: 1;">
                <?php echo 'เรื่อง ' . '<u>' . 'รายงานขอชื้อยา'; ?>
    ...
    <table width="100%" cellspacing="0" cellpadding="0" border="0">
        <tr>
            <td style="font-size: 16pt;line-height: 1;" width="100%">
                ...
                <?php echo 'ตามที่กลุ่มงานเภสัชกรรม โรงพยาบาลมะเร็งอุดรธานี ได้รับอนุมัติแผนการจัดชื้อยาและเวชภัณฑ์มิใช่ยา ประจำปีงบประมาณ ๒๕๕๘ แล้วนั้น  บัดนี้จึงใคร่ขอจัดทำรายงานขอชื้อยาและเวชภัณฑ์มิใช่ยา โดยมีรายละเอียดดังนี้'; ?>
            </td>
        </tr>
        <tr>
                    ...
                    <tr>
                        <td style="font-size: 16pt;line-height: 1;">
                            <?php foreach ($detail as $v) : ?>
                                <p style="line-height: 1;"> <?php echo Yii::$app->report->Genndsp(9) . Yii::$app->numberthai->arabicToThaiNumber($j) . '.&nbsp;' . $v->ItemName . ' จำนวน ' . Yii::$app->numberthai->arabicToThaiNumber(number_format($v->PRQty)) . ' ราคา ' . Yii::$app->numberthai->arabicToThaiNumber(number_format($v->PRUnitCost, 4)) . ' บาท'; ?></p>
                                <?php $j++; ?>
                            <?php endforeach; ?>
                        </td>
                    </tr>
        ...
        <tr>
            <td style="font-size: 16pt;line-height: 1;padding-left: 5px;">
                ...
                <?php foreach ($detail as $v) : ?>
                    <p> <?php echo Yii::$app->report->Genndsp(9) . Yii::$app->numberthai->arabicToThaiNumber($k) . '.&nbsp;' . $v->ItemName . ' ราคา ' . Yii::$app->numberthai->arabicToThaiNumber(number_format($v->PRLastUnitCost, 4)) . ' บาท' . ' ' . $v->PRUnit; ?></p>
                    <?php $k++; ?>
                <?php endforeach; ?>
    ...
    <br>
    <p style="font-size: 16pt; line-height: 1;"><?php echo Yii::$app->report->Genndsp(5); ?>
       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo 'จึงเรียนมาเพื่อโปรดพิจารณาหากเห็นชอบขอได้โปรดอนุมัติให้ดำเนินการจัดชื้อยาโดยวิธีตกลงประกวดราคาตาม<br>รายละเอียดในรายงานขอชื้อยาดังกล่าวข้างต้น'; ?></p>
...
<?php if ($type == 'c' && $model['PRTypeID'] == '3' || $model['PRTypeID'] == '5' || $model['PRTypeID'] == '8') : ?>
    ...
    <br>
    <p style="font-size: 16pt; line-height: 1;"><?php echo Yii::$app->report->Genndsp(5); ?>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo 'จึงเรียนมาเพื่อโปรดพิจารณาหากเห็นชอบขอได้โปรดอนุมัติให้ดำเนินการจัดชื้อเวชภัณฑ์โดยวิธีตกลงประกวดราคาตาม<br>รายละเอียดในรายงานขอชื้อเวชภัณฑ์ดังกล่าวข้างต้น'; ?>
    </p>
