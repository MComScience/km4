...
class TbCpoe extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            [['cpoe_id'], 'required'],
            [['cpoe_id', 'cpoe_schedule_type', 'cpoe_type', 'pt_vn_number', 'cpoe_order_by', 'cpoe_order_section', 'cpoe_status', 'cpoe_createby', 'pt_trp_chemo_id', 'chemo_regimen_ids', 'chemo_cycle_seq', 'chemo_cycle_day','pt_trp_regimen_name'], 'safe'],
            [['cpoe_date'], 'safe'],
    ...
    public function getPhysician(){
        $model = \dektrium\user\models\Profile::findOne(Yii::$app->user->getId());
        if($model->User_jobid == '3'){
            return 'Yes';
        }  else {
            return 'No';
        }
