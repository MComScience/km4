...
<div class="row">
    <div class="col-xs-12 col-sm-6 col-md-12">
        <h5 class="success">
            <b><?= Html::encode('Base Solution :'); ?></b> <?= Html::a('<i class="glyphicon glyphicon-plus"></i>Add', ['create-base', 'drugset_id' => $drugset_id, 'vn_number' => $vn, 'drugset_ids' => $drugset_ids], ['class' => 'btn btn-success', 'role' => 'modal-remote']); ?>
        </h5>
        <div class="well">
        ...
        <h5 class="success">
            <b><?= Html::encode('Drug Additive :') ?></b> <?= Html::a('<i class="glyphicon glyphicon-plus"></i>Add', ['create-additive', 'drugset_id' => $drugset_id, 'vn_number' => $vn, 'drugset_ids' => $drugset_ids], ['class' => 'btn btn-success', 'role' => 'modal-remote']); ?>
        </h5>
        <div class="well">
            <div id="detailsDrugAdditive"></div>  
        </div>
        <h5 class="success">
            <b><?= Html::encode('Schedule & Instruction :') ?></b> <?= Html::a('<i class="glyphicon glyphicon-plus"></i>Edit', ['update-instruction', 'drugset_id' => $drugset_id, 'vn_number' => $vn, 'drugset_ids' => $drugset_ids], ['class' => 'btn btn-success', 'role' => 'modal-remote']); ?>
        </h5>
        <ul class="list-group">
...
<script>
    function GettbBasesolution() {
        ...
        $.ajax({
            url: "index.php?r=chemo/pttrp/gettb-basesolution",
            type: "POST",
            data: {parent: parent},
            dataType: "JSON",
    ...
    function GettbDrugAdditive() {
        var parent = <?= "'" . $drugset_ids . "'"; ?>;
        $.ajax({
            url: "index.php?r=chemo/pttrp/gettb-drugadditive",
            type: "POST",
            data: {parent: parent},
            dataType: "JSON",
            success: function (result) {
    ...
    function SaveIVSolutionitem(id) {
        ...
        $.post(
                'index.php?r=chemo/pttrp/saveitem-ivsolution',
                {
