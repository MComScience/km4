...
#use app\modules\pr\models\TbPritemdetail2TempSearch;
...
use app\modules\pr\models\VwItemListGpu;
use app\modules\pr\models\VwGpuplanDetailAvalible;
...
/**
 * GpuController implements the CRUD actions for TbPr2Temp model.
 */
class GpuController extends Controller {
    ...
    public function actionDetails() {
        if (isset($_POST['expandRowKey'])) {
            return $this->renderPartial('view', ['model' => $this->findModel($_POST['expandRowKey']),]);
        } else {
        ...
        if ($model->load($post)) {
            ...
            $dataProvider = new ActiveDataProvider([
                'query' => TbPritemdetail2Temp::find()
                        ->from(['tb_pritemdetail2_temp use index (PRID)'])
                        ->select(['TMTID_GPU', 'ItemName', 'ids', 'PRItemOnPCPlan', 'PROrderQty', 'PRUnitCost', 'PRExtendedCost'])
                        ->where(['PRID' => $id]),
                'pagination' => [
                    'pageSize' => false,
                ],
    ...
    public function actionGetTableGpu() {
        ...
        if ($request->isAjax) {
            ...
            $dataProvider = new ActiveDataProvider([
                'query' => VwItemListGpuplanAvalible::find()
                        ->select(['ItemID', 'TMTID_GPU', 'FSN_GPU', 'PCPlanNum', 'GPUOrderQty', 'DispUnit', 'PRApprovedOrderQty', 'PRGPUAvalible']),
                'pagination' => [
                    'pageSize' => false,
                ],
                'sort' => [
            ...
            $table = GridView::widget([
                        ...
                        'bordered' => false,
                        'responsive' => false,
                        'tableOptions' => ['class' => GridView::TYPE_DEFAULT, 'id' => 'datatable-gpu', 'width' => '100%'],
                        'layout' => '{items}',
                        'columns' => [
                            [
                            ...
                            [
                                ...
                                'format' => ['decimal', 2],
                                'value' => function ($model) {
                                    return empty($model->PRGPUAvalible) ? '' : $model->PRGPUAvalible;
                                },
                            ],
                            [
    ...
    public function actionAddItemdetail() {
        ...
        if ($request->isAjax) {
            $duplicate = $this->CheckDuplicate($request->post('gpu'), $request->post('PRID'), $request->post('plan'));
            if (!empty($duplicate)) {
                return 'false';
            }
            $itemList = VwItemListGpu::findOne(['TMTID_GPU' => $request->post('gpu'), 'ItemID' => $request->post('ItemID')]);
            $ItemPackUnit = $this->getItemPackUnit($request->post('gpu'), $request->post('ItemID'));

            return $this->renderAjax('_form_detailgpu', [
                        'model' => $model,
    ...
    public function actionAddItemdetailReject() {
        ...
        if ($request->isAjax) {
            $duplicate = $this->CheckDuplicateReject($request->post('gpu'), $request->post('PRID'), $request->post('plan'));
            if (!empty($duplicate)) {
                return 'false';
            }
            $itemList = VwItemListGpu::findOne(['TMTID_GPU' => $request->post('gpu'), 'ItemID' => $request->post('ItemID')]);
    ...
    private function CheckDuplicate($gpu, $prid, $plan) {
        if (empty($plan)) {
            $Check = TbPritemdetail2Temp::findAll(['TMTID_GPU' => $gpu, 'PRID' => $prid]);
        } else {
            $Check = TbPritemdetail2Temp::findAll(['TMTID_GPU' => $gpu, 'PRID' => $prid, 'PCPlanNum' => $plan]);
        }
        return $Check;
    }

    private function CheckDuplicateReject($gpu, $prid, $plan) {
        if (empty($plan)) {
            $Check = TbPritemdetail2::findAll(['TMTID_GPU' => $gpu, 'PRID' => $prid]);
        } else {
            $Check = TbPritemdetail2::findAll(['TMTID_GPU' => $gpu, 'PRID' => $prid, 'PCPlanNum' => $plan]);
        }
        return $Check;
    }

    private function getItemPackUnit($gpu, $ItemID) {
        $query = TbItempack::find()->where(['TMTID_GPU' => $gpu])->all();
        foreach ($query as $d) {
    ...
    public function actionGetDatapcplan() {
        ...
        if ($request->isAjax) {
            Yii::$app->response->format = Response::FORMAT_JSON;
            $query = VwGpuplanDetailAvalible::findOne([
                        'TMTID_GPU' => $request->post('TMTID_GPU'),
    ...
    private function CheckOverPlan($PRItemStdCost, $PRItemUnitCost, $PRUnitCost, $PRItemAvalible, $PROrderQty) {
        ...
        if (($STD == 'StdOver') && ($Unicost == 'UnicostNotOver') && ($Qty == 'QtyNotOver')) {
            ...
            #ราคาต่อหน่วยเกินราคาต่อหน่วยในแผน
    ...
    public function actionUpdateDetailgpu($id) {
        ...
        if ($request->isGet) {
            ...
            $itemList = VwItemListGpu::findOne(['TMTID_GPU' => $model['TMTID_GPU'], 'ItemID' => $model['ItemID']]);
    ...
    private function getReasonLabel($id) {
        ...
        if ($ReasonSelected != null) {
            ...
            foreach ($ReasonSelected as $reason) {
                $queryReason = TbPrReason::find()->where(['PRTypeID' => '1', 'ids' => $reason['PRreasonID']])->orderBy('ids ASC')->one();
                if ($queryReason != null) {
