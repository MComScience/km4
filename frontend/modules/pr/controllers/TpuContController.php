...
/**
 * TpuContController implements the CRUD actions for TbPr2Temp model.
 */
class TpuContController extends Controller {
    ...
    protected function getReasonPR($id) {
        $queryReason = TbPrReason::find()->where(['PRTypeID' => '4'])->orderBy('ids ASC')->all();
        $no = 1;
        $html = null;
    ...
    public function actionSaveHeader() {
        ...
        if ($request->isAjax) {
            ...
            $model->PRDate = empty($post['TbPr2Temp']['PRDate']) ? '' : Yii::$app->dateconvert->convertThaiToMysqlDate2($post['TbPr2Temp']['PRDate']);
            $model->POContactNum = empty($post['TbPr2Temp']['POContactNum']) ? '' : $post['TbPr2Temp']['POContactNum'];
            $model->DepartmentID = empty($post['TbPr2Temp']['DepartmentID']) ? '' : $post['TbPr2Temp']['DepartmentID'];
            $model->SectionID = empty($post['TbPr2Temp']['SectionID']) ? '' : $post['TbPr2Temp']['SectionID'];
            $model->PRTypeID = 4;
            $model->POTypeID = 3;
            $model->PRExpectDate = empty($post['TbPr2Temp']['PRExpectDate']) ? null : $post['TbPr2Temp']['PRExpectDate'];
            $model->PRReasonNote = empty($post['TbPr2Temp']['PRReasonNote']) ? null : $post['TbPr2Temp']['PRReasonNote'];
    ...
    private function getReasonLabel($id) {
        ...
        if ($ReasonSelected != null) {
            ...
            foreach ($ReasonSelected as $reason) {
                $queryReason = TbPrReason::find()->where(['PRTypeID' => '4', 'ids' => $reason['PRreasonID']])->orderBy('ids ASC')->one();
                if ($queryReason != null) {
        ...
        if (isset($_POST['expandRowKey'])) {
            ...
            if (!empty($query['PCPlanNum'])) {
                $dataProvider1 = new SqlDataProvider([
                    'sql' => 'SELECT
                        (SELECT SUM(tb_pcplantpudetail.TPUOrderQty) FROM tb_pcplantpudetail WHERE tb_pcplantpudetail.TMTID_TPU = :TMTID_TPU AND tb_pcplantpudetail.PCPlanNum = :PCPlanNum) AS plan_qty,
                        ...
                        (ifnull((SELECT SUM(tb_pcplantpudetail.TPUOrderQty)
                                                ...
                                                WHERE tb_pcplantpudetail.TMTID_TPU = :TMTID_TPU AND tb_pcplantpudetail.PCPlanNum = :PCPlanNum),0) 
                ...
                GROUP BY
                        plan_qty',
                    'params' => [':TMTID_GPU' => $query['TMTID_GPU'], ':TMTID_TPU' => $query['TMTID_TPU'], ':PRID' => $query['PRID'], ':PCPlanNum' => $query['PCPlanNum']],
                ]);
            } else {
                $dataProvider1 = new SqlDataProvider([
                    'sql' => 'SELECT
