...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <?php // echo $this->render('config', ['action' => 'update-verify-tpu']); ?>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                ...
                $form->field($model, 'PCPlanNum', ['showLabels' => false])->widget(Select2::classname(), [
                    'data' => ArrayHelper::map(VwTpuplanDetailAvalible::find()->where(['TMTID_TPU' => !empty($model['TMTID_TPU']) ? $model['TMTID_TPU'] : $itemList['TMTID_TPU'],'PCPlanTypeID' => 5])->all(), 'PCPlanNum', 'PCPlanNum'),
                    'language' => 'en',
                    'pluginOptions' => [
        ...
        <div class="form-group">
            ...
            <?= Html::activeLabel($model, 'PROrderQty', ['label' => 'ขอซื้อ' . '<span id="checkขอซื้อ"></span>', 'class' => 'col-sm-2 control-label no-padding-right']) ?>
            <div class="col-sm-3">
                <?=
...
<script type="text/javascript">
    ...
    $('#tbpritemdetail2-pcplannum').on('change', function () {
        var TMTID_TPU = $("#tbpritemdetail2-tmtid_tpu").val();
        ...
        if (PCPlanNum !== null) {
            $.ajax({
                ...
                success: function (data) {
                    $('#tbpritemdetail2-itemname').val(data.FSN_TMT);
                    $('#tbpritemdetail2-pritemstdcost').val(data.GPUStdCost);
                    $('#tbpritemdetail2-pritemunitcost').val(data.TPUUnitCost);
                    $('#tbpritemdetail2-pritemorderqty').val(data.TPUOrderQty);
                    $('#tbpritemdetail2-prapprovedorderqtysum').val(data.PRApprovedOrderQty);
                    $('#tbpritemdetail2-pritemavalible').val(data.PRTPUAvalible);
                }
            });
        } else {
                        ...
                        swal({
                            ...
                            closeOnCancel: true,
                                ...
                                function (isConfirm) {
                                    if (isConfirm) {
                                        l.ladda('stop');
                                        $.pjax.reload({container: '#verify_tpu_pjax'});
