...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <?= $form->field($model, 'TMTID_TPU', ['showLabels' => false])->hiddenInput(['value' => !empty($model['TMTID_TPU']) ? $model['TMTID_TPU'] : $itemList['TMTID_TPU']]); ?>
            </div>
        </div>
...
<script type="text/javascript">
        ...
        if (PCPlanNum !== null) {
            $.ajax({
                ...
                success: function (data) {
                    $('#tbpritemdetail2temp-itemname').val(data.FSN_GPU);
                    $('#tbpritemdetail2temp-pritemstdcost').val(data.GPUStdCost);
                    $('#tbpritemdetail2temp-pritemunitcost').val(data.GPUUnitCost);
                    $('#tbpritemdetail2temp-pritemorderqty').val(data.GPUOrderQty);
                    $('#tbpritemdetail2temp-prapprovedorderqtysum').val(data.PRApprovedOrderQty);
                    $('#tbpritemdetail2temp-pritemavalible').val(data.PRGPUAvalible);
                }
            });
        } else {
    ...
    $('#tbpritemdetail2temp-itempackid').on('change', function () {
        ...
        if (ItemPackUnit !== null) {
            ...
            $('#tbpritemdetail2temp-prorderqty').val(null);
            $("#tbpritemdetail2temp-prunitcost").val(null);
            $("#tbpritemdetail2temp-prextendedcost").val(null);
            $("#tbpritemdetail2temp-itempackcost").val(null);
        }
    });
