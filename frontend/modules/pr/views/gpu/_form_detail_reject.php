...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12">
        <?php $form = ActiveForm::begin(['type' => ActiveForm::TYPE_HORIZONTAL, 'id' => 'formitemdetails', 'action' => Url::to(['save-itemdetails-reject']),]); ?>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                ...
                $form->field($model, 'TMTID_GPU', ['showLabels' => false])->textInput([
                    'readonly' => true,
                    'value' => !empty($model['TMTID_GPU']) ? $model['TMTID_GPU'] : $itemList['TMTID_GPU'],
                ]);
                ?>
            </div>
            ...
            <div class="col-sm-2">
                <?php // echo $this->render('config', ['action' => 'update-detail-gpu']); ?>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                ...
                $form->field($model, 'PCPlanNum', ['showLabels' => false])->widget(Select2::classname(), [
                    'data' => ArrayHelper::map(VwGpuplanDetailAvalible::find()->where(['TMTID_GPU' => !empty($model['TMTID_GPU']) ? $model['TMTID_GPU'] : $itemList['TMTID_GPU']])->all(), 'PCPlanNum', 'PCPlanNum'),
                    'language' => 'en',
                    'pluginOptions' => [
        ...
        <div class="form-group">
            <div class="col-sm-2">
                ...
                <?= $form->field($model, 'TMTID_TPU', ['showLabels' => false])->hiddenInput(['value' => $itemList['TMTID_TPU']]); ?>
                <input id="checkform-submit" value="" name="formsubmit" class="form-control" type="hidden"/>
            </div>
...
<script type="text/javascript">
    ...
    function addTotal(total) {
        if (total > 0) {
            ...
            $("#tbpritemdetail2-prextendedcost").val('0.00');//รวมเป็นเงิน
        }
    }
    //คำนวณจำนวนแพค
    ...
    $("#tbpritemdetail2-itempackcost").keyup(function () {/* ราคา/แพค */
        ...
        var prorderqty = parseFloat($("#tbpritemdetail2-prorderqty").val().replace(/[,]/g, "")) || 0;/* ขอซื้อ */
        // var prunitcost = parseFloat($("#tbpritemdetail2-prunitcost").val().replace(/[,]/g, ""));//ราคาต่อหน่วย
    ...
    $('#tbpritemdetail2-pcplannum').on('change', function () {
        var TMTID_GPU = $("#tbpritemdetail2-tmtid_gpu").val();
        ...
        if (PCPlanNum !== null) {
            $.ajax({
                ...
                success: function (data) {
                    $('#tbpritemdetail2-itemname').val(data.FSN_GPU);
                    $('#tbpritemdetail2-pritemstdcost').val(data.GPUStdCost);
                    $('#tbpritemdetail2-pritemunitcost').val(data.GPUUnitCost);
                    $('#tbpritemdetail2-pritemorderqty').val(data.GPUOrderQty);
                    $('#tbpritemdetail2-prapprovedorderqtysum').val(data.PRApprovedOrderQty);
                    $('#tbpritemdetail2-pritemavalible').val(data.PRGPUAvalible);
                }
            });
        } else {
    ...
    $('#tbpritemdetail2-itempackid').on('change', function () {
        ...
        if (ItemPackUnit !== null) {
            $.ajax({
                ...
                type: "POST",
                data: {ItemPackUnit: ItemPackUnit, TMTID_GPU: TMTID_GPU, PRPackQty: PRPackQty, ItemPackCost: ItemPackCost, Type: Type},
                dataType: 'json',
