                        ...
                        [
                            ...
                            'urlCreator' => function ($action, $model, $key, $index) {
                                if ($model->PRTypeID == 1) {//ยาสามัญ
                                    if ($action === 'detail') {
                                        return Url::to(['/pr/gpu/view-reject-verify', 'data' => $key]);
                                    }
                                    if ($action === 'edit') {
                                        return Url::to(['/pr/gpu/update-reject-verify', 'data' => $key]);
                                    }
                                } elseif ($model->PRTypeID == 2) {//ยาการค้า
                                    if ($action === 'detail') {
                                        return Url::to(['/pr/tpu/view-reject-verify', 'data' => $key]);
                                    }
                                    if ($action === 'edit') {
                                        return Url::to(['/pr/tpu/update-reject-verify', 'data' => $key]);
                                    }
                                } elseif ($model->PRTypeID == 3) {//เวชภัณฑ์
                                    if ($action === 'detail') {
                                        return Url::to(['/pr/nd/view-reject-verify', 'data' => $key]);
                                    }
                                    if ($action === 'edit') {
                                        return Url::to(['/pr/nd/update-reject-verify', 'data' => $key]);
                                    }
                                } elseif ($model->PRTypeID == 4) {
                                    if ($action === 'detail') {
                                        return Url::to(['/pr/tpu-cont/view-reject-verify', 'data' => $key]);
                                    }
                                    if ($action === 'edit') {
                                        return Url::to(['/pr/tpu-cont/update-reject-verify', 'data' => $key]);
                                    }
                                } elseif ($model->PRTypeID == 5) {
                                    if ($action === 'detail') {
                                        return Url::to(['/pr/nd-cont/view-reject-verify', 'data' => $key]);
                                    }
                                    if ($action === 'edit') {
                                        return Url::to(['/pr/nd-cont/update-reject-verify', 'data' => $key]);
                                    }
                                }
                            }
                        ],
