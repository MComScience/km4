...
if ($type == 'edit' || $type == 'new') {
    $ButtonSelect = Html::a(Icon::show('plus') . 'เลือกรายการเวชภัณฑ์', false, ['class' => 'btn btn-success', 'onclick' => 'GetModalTableND(this);', 'title' => 'เลือกรายการเวชภัณฑ์']);
    ...
    $ButtonActions = [
        ...
        'buttons' => [
            'update' => function ($url, $model, $key) {
                 ...
                 * */
                return Html::button(Icon::show('edit') . Yii::t('app', 'Edit'), [
    ...
    $ButtonSelect = Html::button(Icon::show('plus') . 'เลือกยาการค้า', ['disabled' => true, 'class' => 'btn btn-success',]);
        ...
        'buttons' => [
            'ok' => function ($url, $model) {
                if ($model->PRVerifyQty > 0.00) {
                    return Html::a(Icon::show('close') . Yii::t('app', 'Cancel'), "#", [
                                'title' => Yii::t('app', 'Cancel'),
                                'class' => 'btn btn-warning btn-xs btn-cancel',
                                'data-toggle' => 'modal',
                    ]);
                } else {
                    return Html::a(Icon::show('check') . Yii::t('app', 'OK'), '#', [
                                'title' => Yii::t('app', 'OK'),
                                'data-toggle' => 'modal',
            ...
            'update' => function ($url, $model, $key) {
                return Html::a(Icon::show('edit') . Yii::t('app', 'Edit'), '#', [
                            'class' => 'btn btn-primary btn-xs activity-update-link',
                            'title' => Yii::t('app', 'Edit'),
                            'data-toggle' => 'modal',
                            'data-id' => $key,
...
<style type="text/css">
    ...
    table#datatable-nd thead tr th{
...
<?php if ($action == 'update' || $action == 'update-reject-verify' || $action == 'view' || $action == 'view-reject-verify') : ?>
    <div class="form-group">
        <div class="col-xs-12 col-sm-12 col-md-12">
            ...
            <?php Pjax::begin(['id' => 'detail_nd_pjax']) ?>
            <?=
            GridView::widget([
...
<?php if ($action == 'verify-pr' || $action == 'view-verify' || $action == 'approve-pr') : ?>

    <div class="form-group">
        <div class="col-sm-12">
            <h5 class="row-title before-success"><?= Html::encode('รายละเอียดใบขอซื้อ'); ?></h5>
            <?php Pjax::begin(['id' => 'verify_nd_pjax']) ?>
            <?=
            GridView::widget([
                'dataProvider' => $dataProvider,
                'showPageSummary' => true,
