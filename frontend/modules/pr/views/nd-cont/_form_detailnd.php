        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                ...
                $form->field($model, 'PCPlanNum', ['showLabels' => false])->widget(Select2::classname(), [
                    'data' => ArrayHelper::map(VwNdplanDetailAvalible::find()->where(['ItemID' => !empty($model['ItemID']) ? $model['ItemID'] : $itemList['ItemID'],'PCPlanTypeID' => 6])->all(), 'PCPlanNum', 'PCPlanNum'),
                    'language' => 'en',
                    'pluginOptions' => [
        ...
        if (PCPlanNum !== null) {
            $.ajax({
                ...
                success: function (data) {
                    $('#tbpritemdetail2temp-itemname').val(data.ItemName);
                    $('#tbpritemdetail2temp-pritemstdcost').val(data.NDStdCost);
                    $('#tbpritemdetail2temp-pritemunitcost').val(data.PCPlanNDUnitCost);
                    $('#tbpritemdetail2temp-pritemorderqty').val(data.PCPlanNDQty);
                    $('#tbpritemdetail2temp-prapprovedorderqtysum').val(data.PRApprovedQtySUM);
                    $('#tbpritemdetail2temp-pritemavalible').val(data.PRNDAvalible);
                }
            });
        } else {
    ...
    $('#tbpritemdetail2temp-itempackid').on('change', function () {
        ...
        if (ItemPackUnit !== null) {
            $.ajax({
                ...
                type: "POST",
                data: {ItemPackUnit: ItemPackUnit, PRPackQty: PRPackQty, ItemPackCost: ItemPackCost, Type: Type, ItemID: ItemID},
                dataType: 'json',
