...
class LendProductController extends Controller
{
    ...
    public function actionClickCancel(){
        if(isset($_POST['STID'])){
            ...
            Yii::$app->getSession()->setFlash('alert1', [
                    ...
                    'title' => Yii::t('app', \yii\helpers\Html::encode('ยกเลิกใบให้ยืมสินค้า')),
                    'message' => Yii::t('app', \yii\helpers\Html::encode('ยกเลิกใบให้ยืมสินค้า'.$findSTNum['STID'].' เรียบร้อยแล้ว')),
                    'positonY' => 'top',
                    'positonX' => 'right'
                ]);
            return '99';
    ...
    {   
        ...
        $dataProvider = $searchModel->Lendsearch(Yii::$app->request->queryParams);
    ...
    public function actionHistoryLend()
    {
        $searchModel = new \app\modules\Inventory\models\TbSt2Search();
        $dataProvider = $searchModel->SearchHistoryLend(Yii::$app->request->queryParams);
        ...
        //     }
        // }
        return $this->render('history-lend', [
                    'searchModel' => $searchModel,
                    'dataProvider' => $dataProvider,
        ]);
    }
    ...
    public function actionCreateLend() {
        $userid = Yii::$app->user->identity->profile->user_id;
        $this->CheckAuto();
        Yii::$app->db->createCommand('CALL cmd_st2_lendproduct_create_header(:x);')
                ->bindParam(':x', $userid)
                ->execute();
    ...
    {   
        ...
        if (Yii::$app->request->post()) {
            ...
            Yii::$app->logger->savelog("บันทึกใบให้ยืมสินค้า",$STID,'ST');
    ...
    {   
        ...
        $dataProvider = $searchModel->SearchHistoryLend(Yii::$app->request->queryParams, $STID);
        $dataProvider->pagination->pageSize = 5;
        if($modelST['STPerson']== NULL ||$modelST['STPerson']==''){
            $vendername = '';
        }else {
        ...
        $htl = '<table class="table table-striped  table-bordered dt-responsive " cellspacing="0" width="100%" id="getdatavendortable">
                            <thead class="bordered-success">
                                <tr>
                                    ...
                                    <th width="30%" style="text-align: center">รหัสผู้ยืม</th>
                                    <th width="100%" style="text-align: center">ชื่อผู้ยืม</th>
  ...
  ';
    ...
    public function actionStockIssue(){
        ...
        $statusID = 20;
        $userid = Yii::$app->user->identity->profile->user_id;
        Yii::$app->logger->savelog("ให้ยืมสินค้า",$STID,'ST'); 
        ...
        return $this->redirect('index.php?r=Inventory/lend-product/index');
    }
    ...
    {
        $STID = $id;
        Yii::$app->logger->savelog("ลบบันทึกใบให้ยืมสินค้า",$STID,'ST'); 
