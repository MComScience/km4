...
class AddmaterialsscienceController extends \yii\web\Controller {

    function actionLab1() {
        return $this->redirect(['create-itemnew', 'type' => 7]);
    }

    function actionLab2() {
        return $this->redirect(['create-itemnew', 'type' => 8]);
    }

    function actionLab3() {
        return $this->redirect(['create-itemnew', 'type' => 9]);
    }

    public function actionIndex() {
        $searchModel = new TbItemSearch();
        $dataProvider = $searchModel->searchmaterialsscience(Yii::$app->request->queryParams);
        $dataProvider->pagination->pageSize = 10;
        $VwItemBalance = VwItemBalance::find()->where('ItemID like "5%"')->count();
