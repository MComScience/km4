 ...
 */
class LendProductGrController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $dataProvider = $searchModel->SearchIndexLend(Yii::$app->request->queryParams);
    ...
    {   
        ...
        $dataProvider = $searchModel->SearchLendTemp(Yii::$app->request->queryParams);
        return $this->render('lend-gr', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }
    public function actionHistory() {
        ...
        return $this->render('history-lend-gr', [
                    'searchModel' => $searchModel,
                    'dataProvider' => $dataProvider,
        ]);
    }
    ...
    public function actionCreateGrtemp($STID) {
        ...
        if($findGr2!=null){
           return $findGr2['GRID'];
            ...
            $this->CheckAuto();
            Yii::$app->db->createCommand('CALL cmd_gr2_lendproduct_create_header_detail(:x,:userid);')
                    ->bindParam(':x', $STID)
    ...
    {   
        ...
        if ($modelGR->load(Yii::$app->request->post())) {
            ...
            Yii::$app->logger->savelog("บันทึกใบรับสินค้าให้ยืม",$GRID,'GR');
...
//        $savedata->save();
    ...
    public function actionReceiveToStock() {
        ...
        if($valueOut =='GO'){
        Yii::$app->logger->savelog("รับสินค้าให้ยืม",$GRID,'GR');    
        ...
        Yii::$app->getSession()->setFlash('alert1', [
            ...
            'duration' => 5000,
            'icon' => 'fa fa-check-square-o',
            'title' => Yii::t('app', \yii\helpers\Html::encode('รับสินค้าให้ยืม')),
    ...
    {   
        Yii::$app->logger->savelog("ลบบันทึกใบรับสินค้าให้ยืม",$id,'GR'); 
        $userid = Yii::$app->user->identity->profile->user_id;
