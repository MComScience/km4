...
class AddcentralizedController extends \yii\web\Controller {
    ...
    function actionCreateheaderipdopd() {
        return $this->redirect(['create-itemnew', 'type' => 6]);
    }

    public function actionIndex() {
        $searchModel = new TbItemSearch();
        $dataProvider = $searchModel->searchcenter(Yii::$app->request->queryParams);
        ...
        $VwItemBalance = VwItemBalance::find()->where('ItemID like "4%"')->count();
    ...
    public function actionCreateItemnew($type = null) {
        $posts = Yii::$app->db->createCommand("SELECT tb_item.ItemID FROM tb_item WHERE tb_item.ItemID LIKE '4%' ORDER BY tb_item.ItemID DESC LIMIT 1")->queryOne();
        if ($posts != null) {
