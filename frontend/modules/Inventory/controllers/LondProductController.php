 ...
 */
class LondProductController extends Controller
{
    ...
    public function actionIndex() {
        ...
        $dataProvider = $searchModel->SearchLond(Yii::$app->request->queryParams);
        return $this->render('index', [
                    'searchModel' => $searchModel,
                    'dataProvider' => $dataProvider,
        ]);
    }
    public function actionHistoryLondproduct() {
        $searchModel = new \app\modules\Inventory\models\TbGr2Search();
        $dataProvider = $searchModel->SearchHistoryLond(Yii::$app->request->queryParams);
        return $this->render('history-londproduct', [
                    'searchModel' => $searchModel,
                    'dataProvider' => $dataProvider,
        ]);
    }
    ...
    {   
        ...
        if ($modelGR->load(Yii::$app->request->post())) {
            ...
            Yii::$app->db->createCommand('CALL cmd_gr2_savedraft_londproduct(:GRID,:GRDate,:VenderInvoiceNum,:VenderID,:GRNote,:StkID);')
                    ->bindParam(':GRID', $GRID)
    ...
    {   
        ...
        if ($modelGR->load(Yii::$app->request->post())) {
            $GRID = $_POST['TbGr2Temp']['GRID'];
            $GRDate = Yii::$app->componentdate->convertThaiToMysqlDate2($_POST['TbGr2Temp']['GRDate']);
            $PODueDate = Yii::$app->componentdate->convertThaiToMysqlDate2($_POST['TbGr2Temp']['PODueDate']);
            $VenderInvoiceNum = $_POST['TbGr2Temp']['VenderInvoiceNum'];
            $VenderID = $_POST['TbGr2Temp']['VenderID'];
            $GRNote= $_POST['TbGr2Temp']['GRNote'];
            $StkID = $_POST['TbGr2Temp']['StkID'];
            Yii::$app->logger->savelog("บันทึกใบรับสินค้าการขอยืม",$GRID,'GR');
            Yii::$app->db->createCommand('CALL cmd_gr2_savedraft_londproduct(:GRID,:GRDate,:PODueDate,:VenderInvoiceNum,:VenderID,:GRNote,:StkID);')
                    ->bindParam(':GRID', $GRID)
                    ->bindParam(':GRDate', $GRDate)
    ...
    public function actionCreateLond() {
        $userid = Yii::$app->user->identity->profile->user_id;
        $this->CheckAuto();
        Yii::$app->db->createCommand('CALL cmd_gr2_londproduct_create_header(:x);')
                ->bindParam(':x', $userid)
                ->execute();
        ...
        $htl = '<table class="table table-striped  table-bordered dt-responsive " cellspacing="0" width="100%" id="getdatavendortable">
                            <thead class="bordered-success">
                                <tr>
                                    ...
                                    <th width="30%" style="text-align: center">รหัสผู้ให้ยืม</th>
                                    <th width="100%" style="text-align: center">ชื่อผู้ให้ยืม</th>
    ...
    function actionGetdataTpu() {
        ...
        //$PODueDate = Yii::$app->componentdate->convertThaiToMysqlDate2($_GET['PODueDate']);
        if($VenderInvoiceNum != ""|| $VenderInvoiceNum != null){
            if( $VenderID !="" ||  $VenderID != null){
                    $sql = "update tb_gr2_temp set VenderInvoiceNum='$VenderInvoiceNum',VenderID='$VenderID' WHERE tb_gr2_temp.GRID = $GRID;";
        ...
        }else if( $VenderID !="" ||  $VenderID != null){
...
//        $savedata->save();
    ...
    public function actionReceiveToStock() {
        ...
        if($valueOut =='GO'){
            Yii::$app->logger->savelog("รับสินค้าการขอยืม",$GRID,'GR');
            ...
            Yii::$app->getSession()->setFlash('alert1', [
                ...
                'duration' => 5000,
                'icon' => 'fa fa-check-square-o',
                'title' => Yii::t('app', \yii\helpers\Html::encode('รับสินค้าการขอยืม')),
                'message' => Yii::t('app', \yii\helpers\Html::encode('รับสิน้าเข้าคลังเรียบร้อยแล้ว!')),
                'positonY' => 'top',
                'positonX' => 'right'
            ]);
            return $this->redirect('index.php?r=Inventory/lond-product/index');
        }else{
            echo 'false';
        }
    }
    ...
    {   
        Yii::$app->logger->savelog("ลบบันทึกใบรับสินค้าการขอยืม",$id,'GR');
