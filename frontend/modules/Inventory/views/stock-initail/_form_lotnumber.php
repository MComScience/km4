...
<!------------------------------------START FROM Goods Receiving------------------------------------>
							<div class="form-group" >
                          	...
                          	<div class="form-group" >
                                <label class="col-sm-1 control-label no-padding-right"><b>ชื่อสินค้า:</b></label>
                                <div class="col-sm-6" style="margin-top: 8px;">
                                    <?php echo $Item['ItemID'] ?>&nbsp;:&nbsp;<?php echo $Item['ItemName'] ?>
								</div>
                            </div>
                        <!-- <div class="form-group">
                            ...
                            <div class="form-group" style="margin-top:8px;">
                               <label class="col-sm-1 control-label no-padding-right"><b>รับครั้งนี้:</b></label>
                                ...
                                <!-- <label class="col-sm-2 control-label no-padding-right">ราคาต่อแพค<a id="checkราคาต่อแพค"></a></label> -->
                                <label class="col-sm-1 control-label no-padding-right">ราคา/แพค<a id="checkราคาต่อแพค"></a></label>
                                <div class="col-sm-2">
                                    <?=
                            ...
                            <div class="form-group" style="margin-top:8px;">
                                ...
                                <label class="col-sm-1 control-label no-padding-right">ราคา/หน่วย<a id="checkราคาต่อหน่วย"></a></label>
                                <div class="col-sm-2">
                                   <?=
                            ...
                            <div class="form-group" >
                                ...
                                <!-- <label class="col-sm-2 control-label no-padding-right">รวมเป็นเงิน</label> -->
                                <div class="col-sm-1">
                                  <?=
                            ...
                            <input type="hidden" class="form-control" name="packid" id="packid" value="<?php echo $packid; ?>"/>
                            <br>
                    ...
                    <div class="well">
                        <div class="tb-pr2-form">
							...
							<div class="form-group" style="text-align: right"><br>
                                <?= Html::a('Close', ['create','GRID' => $GRID,'view' => ''], ['class' => 'btn btn-default']) ?>
...
$(document).ready(function () {
    //document.getElementById("SaveLotNumber").disabled = true; 
     var packid = $("#packid").val();
     if(packid == ''){
        
     }else{
       $("#vwgr2detail-gritempackid").val(packid).trigger("change");  
     }
     fn_check_btn_receive();
...
//--------------------------------START Pack/Unit Assign Lot NUmber----------------------------------
        if ($("input[id=ชิ้นlot]").is(":checked")){
                ...
                $('#checkจำนวนแพคlot').html('<font color="red">*</font>');
                $('#checkจำนวนlot').html('');
...
//----------------------------------START Pack/Unit Goods Receiving----------------------------------------
        var CheckPack = $("#vwgr2detail-gritempackskuqty").val();
        if (CheckPack == "" || CheckPack == "0.00") {
            document.getElementById("ชิ้น").checked = true;
            if ($("input[id=ชิ้น]").is(":checked"))
            {
                $('#checkจำนวน,#checkราคาต่อหน่วย').html('<font color="red">*</font>');
    ...
    $("input[id=แพค]").click(function () {
        if ($(this).is(":checked"))
        {
             $("#vwgr2detail-grpackqty,#vwgr2detail-grpackunitcost").removeAttr('readonly');
                ...
                $("#vwgr2detail-gritemqty,#vwgr2detail-gritemunitcost").attr('readonly', 'readonly');
                $('#checkจำนวนแพค,#checkหน่วยแพค,#checkราคาต่อแพค').html('<font color="red">*</font>');
                $('#checkราคาต่อหน่วย,#checkจำนวน').html('');
                $('#vwgr2detail-grpackqty,#vwgr2detail-grpackunitcost').css('background-color', '#FFFF99');
                $('#vwgr2detail-gritemqty,#vwgr2detail-gritemunitcost').css('background-color', 'white');
                fn_check_pack();
        }
...
//------------------------------------START SUBMIT FROM GOODS RECEIVING----------------------------------
  $('#form_goods_receiving').on('beforeSubmit', function(e){ 
      ...
      $("#GRNum_lot").val(GRNum);
      if ($("input[id=แพค]").is(":checked")){
        if($('#vwgr2detail-gritempackid').val()==''){
             swal('ไม่มีขนาดแพค','','warning');
             return false;
        }
        waitGR();
        var \$form = $(this);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.LNAssignedLeftQty == "0.00") {
                    document.getElementById("SaveLotNumber").disabled = false;
                    document.getElementById("btn_assignlot").disabled = true;
                    if ($('#แพคlot').is(':checked')) {
                        $('#tbgritemdetail2temp-grpackqty,#tbgritemdetail2temp-grpackunitcost,#vwgr2lotassigneddetail-itemexternallotnum,#vwgr2lotassigneddetail-itemexpdate').css('background-color', 'white');
                        $('#tbgritemdetail2temp-grpackqty,#tbgritemdetail2temp-grpackunitcost,#tbgritemdetail2temp-gritempackskuqty').val('0.00');
...
//--------------------------------------START Save Lot Number--------------------------------------------      
    ...
    function fn_check_item(){
        $('#vwgr2detail-grpackqty,#vwgr2detail-grpackunitcost').val('0.00');
        fn_check_btn_receive();
    }
    function fn_check_pack(){
        $('#vwgr2detail-gritemqty,#vwgr2detail-gritemunitcost').val('0.00');
        fn_check_btn_receive();
    }
//---------------------------------------START Save Lot Number--------------------------------------------
JS;
