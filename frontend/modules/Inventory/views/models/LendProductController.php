...
class LendProductController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($find != null) {
            ...
            foreach ($STID as $key) {
                $sql = "DELETE FROM tb_st2_temp WHERE STID = $key;
                       DELETE FROM tb_stitemdetail2_temp WHERE STID = $key;";
    ...
    public function actionWaitItem()
    {
        $searchModel = new \app\modules\Inventory\models\VwStListDraftSearch();
        $dataProvider = $searchModel->waititemsearch(Yii::$app->request->queryParams);
        $find = TbSt2Temp::find()->where(['STNum' => NULL])->all();
        if ($find != null) {
            foreach ($find as $data) {
                $STID[] = $data['STID'];
    ...
    public function actionCreateClaim() {
        ...
        Yii::$app->db->createCommand('CALL cmd_st2_lendproduct_create_header(:x,:STID);')
                ->bindParam(':x', $userid)
                ->bindParam(':STID', $max)
                ->execute();
        $model = $this->findModel($max);
        return $this->redirect(['create','STID' => $model->STID, 'view' => '']);
    }
    ...
    {   
        ...
        if (Yii::$app->request->post()) {
            ...
            $STDate = Yii::$app->componentdate->convertThaiToMysqlDate($_POST['TbSt2Temp']['STDate']);
            ...
            $STDueDate = Yii::$app->componentdate->convertThaiToMysqlDate($_POST['TbSt2Temp']['STDueDate']);
        ...
        if (Yii::$app->request->post()){
            if ($_POST['VwSt2DetailSub']['STPackUnit'] != NULL || $_POST['VwSt2DetailSub']['STPackUnit'] =='' ) {
               ...
               $STItemPackID = '';
                ...
                Yii::$app->db->createCommand('CALL cmd_st2_item_save(:ids,:STNum,:STID,:ItemID,:ItemInternalLotNum,:STPackQty,:STPackUnitCost,:STItemPackID,:STItemQty,:STCreatedBy,:STItemUnitCost);')
                    ->bindParam(':ids', $ids)
                    ->bindParam(':STNum', $STNum)
                    ->bindParam(':STID', $STID)
