...
<?php $form = ActiveForm::begin(['type' => ActiveForm::TYPE_HORIZONTAL, 'id' => 'formnd']); ?>
<div class="well">
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <?=
                    $form->field($vwmodel, 'NDStdCost', ['showLabels' => false])->textInput([
                        'style' => 'background-color: white;text-align:right',
                        'value' => number_format($vwmodel['NDStdCost'], 2),
                        'readonly' => true
                    ]);
                    ?>
                </div> 
...
$('#formnd').on('beforeSubmit', function(e) 
{
   var \$form = $(this);
    $.post(
...
$("#tbprselecteddetail-prunitcost").keyup(function () { 
    ...
    if (orq > 0) {
        uni = uni.toFixed(2);
        $("#vwndplandetailprselected-prextenedcost").val(addCommas(jj.toFixed(2)));
    } else {
        $("#vwndplandetailprselected-prextenedcost").val('0.00');
    }
    ...
    $.ajax({
        ...
        if(data.qty == 0){
          $('#tbprselecteddetail-prqty').val(data.ItemPackSKUQty);
            ...
            $("#vwndplandetailprselected-prextenedcost").val(addCommas(Total.toFixed(2)));
        }
...
$("#tbprselecteddetail-itempackcost").keyup(function () {
    ...
    var jj = uni / orq;
    var ext = qty * jj;
    $("#vwndplandetailprselected-prextenedcost").val(addCommas(ext.toFixed(2)));
    if (uni > 0) {
        orq = orq.toFixed(2);
