...
//use kartik\select2\Select2;
?>
<?php $form = ActiveForm::begin(['type' => ActiveForm::TYPE_HORIZONTAL, 'id' => 'formplantpu']); ?>
<div class="well">
    <div class="row">
        <input value="<?php echo $vwmodel['ItemID']; ?>" id="ItemID" type="hidden"/>
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    ...
                    $form->field($vwmodel, 'PRExtenedCost', ['showLabels' => false])->textInput([
                        'style' => 'background-color: white;text-align:right',
                        'value' => number_format($model->PRQty * $model->PRUnitCost, 2),
                        'readonly' => true
                    ]);
                    ?>
                </div>
...
$('#formplantpu').on('beforeSubmit', function(e) 
{
   var \$form = $(this);
    $.post(
        ...
        if(result == 1)
        {
            ...
            $.pjax.reload({container:'#detailplantpu'});
...
$("#tbprselecteddetail-prunitcost").keyup(function () { 
    ...
    if (orq > 0) {
        uni = uni.toFixed(2);
        $("#vwtpuplandetailprselectedpocont-prextenedcost").val(addCommas(jj.toFixed(2)));
    } else {
        $("#vwtpuplandetailprselectedpocont-prextenedcost").val('0.00');
    }
...
$(document).ready(function () {     
 ...
 $('#tbprselecteddetail-itempackid').on('change', function () {
    ...
    $.ajax({
        url: "index.php?r=Inventory/stocks-balance/get-qtyplantpu",
        type: "post",
        ...
        if(data.qty == 0){
          $('#tbprselecteddetail-prqty').val(data.ItemPackSKUQty);
            ...
            $("#vwtpuplandetailprselectedpocont-prextenedcost").val(addCommas(Total.toFixed(2)));
        }
...
$("#tbprselecteddetail-itempackcost").keyup(function () {
    ...
    var jj = uni / orq;
    var ext = qty * jj;
    $("#vwtpuplandetailprselectedpocont-prextenedcost").val(addCommas(ext.toFixed(2)));
    if (uni > 0) {
        orq = orq.toFixed(2);
