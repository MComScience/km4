...
/**
 * TbPo2TempSearch represents the model behind the search form about `app\modules\po\models\TbPo2Temp`.
 */
class TbPo2TempSearch extends TbPo2Temp {
     ...
     */
    public function search_draft($params) {
        $query = TbPo2Temp::find()
                ->select(['PONum', 'PODate', 'POID', 'tb_po2_temp.POTypeID', 'tb_po2_temp.POStatus', 'PODueDate','CONVERT(SUBSTRING_INDEX((SUBSTRING_INDEX(tb_po2_temp.PONum,"/",1)),"ย",-1), UNSIGNED INTEGER) AS N'])
                ->leftJoin('tb_potype', '`tb_potype`.`POTypeID` = `tb_po2_temp`.`POTypeID`')
                ->leftJoin('tb_postatus', '`tb_postatus`.`POStatusID` = `tb_po2_temp`.`POStatus`')
                ->orderBy('N ASC');
