 ...
 *
 * @property TbPoitemdetail2[] $tbPoitemdetail2s
 */
class TbPo2 extends \yii\db\ActiveRecord {
    ...
    public function rules() {
        return [
                [['POTotal', 'PODate', 'PODueDate', 'POCreateDate', 'POCreateTime', 'POVerifyDate', 'POApproveDate', 'PORejectVerifyDate', 'PORejectApproveDate', 'VendorName', 'MenuVendorName', 'POStatus',], 'safe'],
                [['DepartmentID', 'SectionID', 'POTypeID', 'POStatus', 'POCreateBy', 'POVerifyBy', 'PORejectVerifyBy', 'PORejectApproveBy', 'PRTypeID', 'POApproveBy'], 'integer'],
                [['PONum', 'PRNum', 'POContID', 'PCPlanNum'], 'string', 'max' => 50],
                [['VendorID', 'Menu_VendorID'], 'string', 'max' => 13],
                [['POSubtotal', 'POVat', 'PORejectReason', 'PORejfromAppNote'], 'string', 'max' => 255],
        ];
    }
     ...
     */
    public function getTbPoitemdetail2s() {
        return $this->hasMany(TbPoitemdetail2::className(), ['POID' => 'POID']);
    }
    ...
    public function getTitle($status, $action) {
        switch (true) {
            case $status == "2" && $action == "view-verify":
                $title = "ใบสั่งซื้อรอการทวนสอบ";
                break;
            case $status == "2" && $action == "verify":
                $title = "ทวนสอบใบสั่งซื้อ";
                break;
            case $status == "4" && $action == "view-reject":
                $title = "ใบสั่งซื้อไม่ผ่านการทวนสอบ";
                break;
            case $status == "4" && $action == "reject":
                $title = "ใบสั่งซื้อไม่ผ่านการทวนสอบ";
                break;
            case $status == "10" && $action == "view-approve":
                $title = "ใบสั่งซื้อรอการอนุมัติ";
                break;
            case $status == "10" && $action == "approve-po":
                $title = "อนุมัติใบสั่งซื้อ";
                break;
            case $status == "11" && $action == "update-approve":
                $title = "ใบสั่งซื้อผ่านการอนุมัติ";
                break;
            default:
                $title = "";
        }
